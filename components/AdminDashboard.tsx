...
import { UserInfo, TestResult } from '../types';
...
const AdminDashboard: React.FC<{ onSignOut: () => void }> = ({ onSignOut }) => {
    const [activeTab, setActiveTab] = useState<'userInfo' | 'results'>('userInfo');
    const [userInfoList, setUserInfoList] = useState<UserInfo[]>([]);
    const [testResults, setTestResults] = useState<TestResult[]>([]);
    const [loading, setLoading] = useState<boolean>(true);
    const [error, setError] = useState<string | null>(null);

    const [userInfoFilter, setUserInfoFilter] = useState('All Users');
    ...
    const fetchData = useCallback(async (isRefresh = false) => {
        ...
        setError(null);
        try {
            const [userInfoResponse, resultsResponse] = await Promise.all([
                supabase.from('user_info').select('*').order('created_at', { ascending: false }),
            ...
            setUserInfoList(userInfoResponse.data || []);
    ...
    const filteredUserInfo = useMemo(() =>
        userInfoFilter === 'All Users'
            ? userInfoList
            : userInfoList.filter(p => p.user_id.startsWith(userInfoFilter)),
        [userInfoList, userInfoFilter]
    );
    ...
    const userInfoCountsByPrefix = useMemo(() => {
        const counts: { [key: string]: number } = {};
        USER_ID_PREFIXES.forEach(prefix => {
            if (prefix === 'All Users') {
                counts[prefix] = userInfoList.length;
            } else {
                counts[prefix] = userInfoList.filter(p => p.user_id.startsWith(prefix)).length;
            }
        });
        return counts;
    }, [userInfoList]);
    ...
    const handleExport = useCallback(() => {
        ...
        if (activeTab === 'userInfo') {
            const filename = `user-info-${userInfoFilter}-${timestamp}.csv`;
            const dataToExport = filteredUserInfo.map(({ user_id, first_name, last_name, email, country, created_at }) => ({ user_id, first_name, last_name, email, country, created_at }));
            exportToCsv(filename, dataToExport);
        } else { // 'results'
            const filename = `test-results-${resultsFilter}-${timestamp}.csv`;
            exportToCsv(filename, filteredResults);
        }
    }, [activeTab, filteredUserInfo, filteredResults, userInfoFilter, resultsFilter]);
        ...
        <div className="min-h-screen bg-gray-900 text-gray-200 font-sans flex flex-col p-4 sm:p-6 lg:p-8">
            <header className="flex justify-between items-center mb-6 pb-4 border-b border-gray-700">
                <div>
                    ...
                    <p className="text-gray-400">User Info & Test Results Overview</p>
                </div>
            ...
            <main className="flex-grow">
                <div className="mb-6">
                    <div className="border-b border-gray-700">
                        <nav className="-mb-px flex space-x-6" aria-label="Tabs">
                            <button onClick={() => setActiveTab('userInfo')} className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm ${activeTab === 'userInfo' ? 'border-lifewood-saffaron text-lifewood-saffaron' : 'border-transparent text-gray-400 hover:text-gray-200 hover:border-gray-500'}`}>
                                User Info ({userInfoList.length})
                            </button>
                ...
                {loading ? <LoaderIcon /> : error ? (
                    ...
                    <div>
                        {activeTab === 'userInfo' && renderTable(['User ID', 'First Name', 'Last Name', 'Email', 'Country', 'Created At'], filteredUserInfo, userInfoFilter, setUserInfoFilter, USER_ID_PREFIXES, userInfoCountsByPrefix)}
